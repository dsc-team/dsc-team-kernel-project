...
#define FASTBOOT_FLAG_ADDR_PHY 0xF0000
#define FASTBOOT_FLAG_SIZE 16
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		if (!strcmp(cmd, "bootloader")) {
			...
			{
				unsigned int * fastboot_flag_addr_vir=NULL;
				
				if (NULL == request_mem_region(FASTBOOT_FLAG_ADDR_PHY, FASTBOOT_FLAG_SIZE, "fastboot_flag")) {
				    printk("FAILED to request_mem_region in pm2.c");
				    msleep(1000);
				} else {
					fastboot_flag_addr_vir=(unsigned int*) ioremap(FASTBOOT_FLAG_ADDR_PHY, FASTBOOT_FLAG_SIZE);
					memcpy_toio(fastboot_flag_addr_vir, "Fast Boot Flag", strlen("Fast Boot Flag"));
					iounmap(fastboot_flag_addr_vir);
				}
			}

...
static void do_read_data(struct work_struct *work)
{
	...
	struct msm_rpc_endpoint *ept;
#if defined(CONFIG_MSM_ONCRPCROUTER_DEBUG)
	struct rpc_request_hdr *rq = NULL;
#endif
	uint32_t pm, mid;
	unsigned long flags;
...
packet_complete:
	...
	D("%s: take read lock on ept %p, xid: %x %s\n", __func__, ept, (rq == NULL) ? 0: ntohl(rq->xid), (rq == NULL) ? "NULL": "OK");
...
#endif
		IO("REPLY to xid=%d(%x) @ %d:%08x (%d bytes)\n",
		   be32_to_cpu(rq->xid), be32_to_cpu(rq->xid), hdr.dst_pid, hdr.dst_cid, count);
	}
...
static int __init rpcrouter_init(void)
{
    ...
    if(!rpc_zero_wqueue) //work queue success
    {

...
static int mddi_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd; 						/* Jagan+ ... Jagan- */

	int ret = 0;

	mfd = platform_get_drvdata(pdev); 					/* Jagan+ ... Jagan- */
...
//		mddi_pdata->mddi_power_save(0);
	...
	pm_qos_update_request(mfd->pm_qos_req, PM_QOS_DEFAULT_VALUE);		/* Jagan+ ... Jagan- */
...
//		mddi_pdata->mddi_power_save(1);
	...
	if (mfd->fbi->var.pixclock > 65000000)
		/* pm_qos_rate should be in Khz */
		pm_qos_rate = mfd->fbi->var.pixclock / 1000 ;
	else
		pm_qos_rate = 65000;
...
static int mddi_probe(struct platform_device *pdev)
{
	...
	mfd->pm_qos_req = pm_qos_add_request(PM_QOS_SYSTEM_BUS_FREQ ,PM_QOS_DEFAULT_VALUE);

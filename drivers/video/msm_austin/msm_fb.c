...
static unsigned char *fbramnew;
static unsigned char *fbramnew_phys;
static int fbramnew_size;
atomic_t hdmiworkaround = ATOMIC_INIT(0);
#endif
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if ((pdev->id == 0) && (pdev->num_resources > 0)) {
		...
		fbramnew_phys = (char *)pdev->resource[1].start;
		fbramnew = ioremap((unsigned long)fbramnew_phys, fbramnew_size);
#endif
...
#ifdef CONFIG_HDMI_BUGFIX
		mfd->fbram0_phys1 = (unsigned long)fbram_phys; 
		mfd->fbram0_phys2 = (unsigned long)fbram_phys+SRC_SIZE;
		mfd->fbram0_vir1 =  fbram; 
		mfd->fbram0_vir2 =  (unsigned char*)(fbram+SRC_SIZE);
		mfd->fbram1_phys1 = (unsigned long)fbramnew_phys; 
		mfd->fbram1_phys2 = (unsigned long)fbramnew_phys+SRC_SIZE; 
		mfd->fbram1_vir1 =  fbramnew; 
		mfd->fbram1_vir2 =  (unsigned char*)(fbramnew+SRC_SIZE);
		printk(KERN_ERR "fbram0, phy=0x%p,fbram0_phys1=0x%lx,fbram0_phys2=0x%lx,vir=%p,fbram0_vir1=%p,fbram0_vir2=%p,size=%d\n",
			fbram_phys,mfd->fbram0_phys1,mfd->fbram0_phys2,
			fbram,mfd->fbram0_vir1,mfd->fbram0_vir2,fbram_size);
		printk(KERN_ERR "fbram1, phy=0x%p,fbram1_phys1=0x%lx,fbram1_phys2=0x%lx,vir=%p,fbram1_vir1=%p,fbram1_vir2=%p,size=%d\n",
			fbramnew_phys,mfd->fbram1_phys1,mfd->fbram1_phys2,
			fbramnew,mfd->fbram1_vir1,mfd->fbram1_vir2,fbramnew_size);
#endif

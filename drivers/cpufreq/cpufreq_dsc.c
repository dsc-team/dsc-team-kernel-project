...
static ssize_t store_ramp_freq(struct kobject *a, struct attribute *b,
                               const char *buf, size_t count)
{
        unsigned int input;
        int ret;
        ...
        mutex_lock(&dbs_mutex);
        dbs_tuners_ins.ramp_freq = input;
        mutex_unlock(&dbs_mutex);
...
define_one_global_rw(ramp_freq);
...
cpufreq_freq_attr_rw_old(ramp_freq);
...
//n0p
//Jump up
		if (this_dbs_info->requested_freq < dbs_tuners_ins.ramp_freq) {
			(this_dbs_info->requested_freq) = dbs_tuners_ins.ramp_freq;
		}

...
void vfe_cmd_release(struct platform_device *dev)
{
	...
	free_irq(ctrl->vfeirq, 0);
// Jagan+
	//Avoid workqueue runs while or after release
...
// Jagan+
	//enable EOF irq to monitor CAMIF stop issue
...
void vfe_reset(void)
{
	// Jagan+
	/*  
	 Move it into vfe_cmd_init()	
	spin_loick_init(&msm_vfe_ctrl_lock);
	*/
	// Jagan-

...
#ifdef CONFIG_HDMI_BUGFIX	
	printk(KERN_ERR "hdmiworkaround value is %d\n",atomic_read(&hdmiworkaround));		
#endif	
#endif	
	return i;
}
...
static int adb_bind_config(struct usb_configuration *c)
{
	...
	if (adb_debug == NULL) {
		printk(KERN_ERR "Sonia::f_adb.c %s Unable to create /proc/adb_debug\n",__func__);
		return -ENOMEM;
	}

	ent = create_proc_entry("mode", 0, adb_debug);
	if (ent == NULL) {
		printk(KERN_ERR "Sonia::f_adb.c %s Unable to create /proc/adb_debug/mode entry\n",__func__);
		ret = -ENOMEM;
	}
	else
	{
		ent->read_proc = adb_debug_read_proc_debug;
		printk(KERN_INFO "Sonia:f_adb.c %s create /proc/adb_debug/mode entry\n",__func__);
	}

...
#define USB_FUNC0_ADB				0	
#define USB_FUNC1_DIAG_ADB			1
#define USB_FUNC2_RNDIS				2
#define USB_FUNC3_RNDIS_ADB_DIAG	3
...
static void android_set_default_product(int pid)
{
	...
	if (qct_usbdiag_debugging)	
	{
		android_set_function_mask(up+=USB_FUNC1_DIAG_ADB);
	}
	else
	{
		android_set_function_mask(up+=USB_FUNC0_ADB);
	}	
}
...
static void android_config_functions(struct usb_function *f, int enable)
{
	...
	if ( enable && (!strcmp(f->name,"rndis")) )
	{
		if (qct_usbdiag_debugging)	
		{
			android_set_function_mask(up+=USB_FUNC3_RNDIS_ADB_DIAG);
		}
		else
		{
			android_set_function_mask(up+=USB_FUNC2_RNDIS);
		}
	}
	else
	{

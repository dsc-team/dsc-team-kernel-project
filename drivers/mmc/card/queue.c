...
static int mmc_queue_thread(void *d)
{
	...
	down(&mq->thread_sem);
#ifdef CONFIG_MMC_DEBUG_SEMA
	dump_stack_to_buf(mq->stack, 1024);
	mq->owner = current;
#endif
...
#ifdef CONFIG_MMC_DEBUG_SEMA
		if (down_timeout(&mq->thread_sem, msecs_to_jiffies(2000))) {
			printk(KERN_ERR "%s: suspend timeout.\n", __func__);
			mq->stack[1023] = 0;
			printk(KERN_ERR "%s: %s\n", __func__, mq->stack);
			if (mq->owner) show_stack(mq->owner, NULL);
		down(&mq->thread_sem);
	}
		else {
			dump_stack_to_buf(mq->stack, 1024);

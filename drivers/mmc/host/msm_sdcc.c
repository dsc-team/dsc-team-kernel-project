...
static void msmsdcc_power(int on)
{
    if (system_rev <= 	TOUCAN_EVT3_Band148)
        return;

    if (on)
    {
        gpio_tlmm_config(GPIO_CFG(106, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_set_value(106, 0);
        gpio_tlmm_config(GPIO_CFG(88, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(89, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(90, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(91, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(92, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(93, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
        printk(KERN_DEBUG "[SDCC] power on\n");
    }
    else
    {
        gpio_tlmm_config(GPIO_CFG(88, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(89, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(90, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(91, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(92, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(93, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_tlmm_config(GPIO_CFG(106, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_set_value(106, 1);
        printk(KERN_DEBUG "[SDCC] power off\n");
    }
}
#endif
    ...
    if (host->plat->status_irq)
    {
        ...
        else
        {
            if (atomic_read(&host->irq_retry) < 50)
            {
                ...
                #ifdef CONFIG_HW_TOUCAN
                if (atomic_read(&host->power_state) == 0)
                {
                    msmsdcc_power(1);
                    atomic_set(&host->power_state, 1);
                }
                #endif
            }
            else
            {
                ...
                #ifdef CONFIG_HW_TOUCAN
                if (atomic_read(&host->power_state) == 1)
                {
                    msmsdcc_power(0);
                    atomic_set(&host->power_state, 0);
                }
                #endif
            }
        }
...
static DEVICE_ATTR(detect, 0664,

...
#define DISCARDING_THRESHOLD 2000
...
static void smd_net_data_handler(unsigned long arg)
{
	...
	for (;;) {
		...
		if (RMNET_IS_MODE_IP(opmode) ? (sz > DISCARDING_THRESHOLD) :
						(sz > (DISCARDING_THRESHOLD + ETH_HLEN))) {
			pr_err("rmnet_recv() discarding %d len (%d mtu)\n",
				sz, RMNET_IS_MODE_IP(opmode) ?
					DISCARDING_THRESHOLD : (DISCARDING_THRESHOLD + ETH_HLEN));
			ptr = 0;
		} else {

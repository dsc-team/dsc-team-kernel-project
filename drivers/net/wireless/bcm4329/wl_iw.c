...
)
{
	...
	temp_ssid.SSID_len = g_specific_ssid.SSID_len;
	for (i = 0; i < 32; i++)
		temp_ssid.SSID[i] = g_specific_ssid.SSID[i];
	if ((error = dev_wlc_ioctl(dev, WLC_SCAN, &temp_ssid, sizeof(temp_ssid)))) {
	...
	g_specific_ssid.SSID_len = temp_ssid.SSID_len;
	for (i = 0; i < 32; i++)
		g_specific_ssid.SSID[i] = temp_ssid.SSID[i];

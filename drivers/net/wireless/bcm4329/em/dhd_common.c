...
#if defined(UNDER_CE) && defined(EXT_STA)
/*
* Remove from scan results all results older than age_limit (in seconds)
*
*/
int
dhd_iscan_prune_aged_results(void *dhdp, long age_limit)
{
	...
	age_limit *=1000; /* Go from seconds to ms */
	...
	while (iscan_cur) {
		...
		if (iscan_aged->time != 0 && age_limit != 0) {
			if (iscan_aged->time + age_limit < curr_time) {
				/* Scan results are too old -> remove */
...
#endif /* #if defined(UNDER_CE) && defined(EXT_STA) */
...
#ifdef NDIS
	/* Special pre-processing for some actions */
	switch (action) {
		case WL_SCAN_ACTION_START:
			/*
			 * Add a timer to make sure we send
			 * a scan confirmation up - no matter what
			 */
			dhd_iscan_add_timer(dhdp);
			break;

		case WL_SCAN_ACTION_ABORT:
			/* First indicate up a failed scan complete */
			/* This has the side effect of removing any pending iscan timer */
...
{
	DHD_INFO(("%s()\n", __FUNCTION__));
	/* first remove any pending timer */
	dhd_iscan_del_timer(dhdp);

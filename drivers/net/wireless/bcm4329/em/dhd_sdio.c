...
#ifdef UNDER_CE
#define DHD_TXMINMAX	20	/* Max tx frames if rx still pending */
#else
...
#ifdef BCMSPI
	void		*ackpkt;		/* pre allocated ACK packet */
	bool		pr61317_war;
	bool		dwordmode;
#endif /* BCMSPI */
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_SVAL(IOV_RXFLOWMODE):
		/* If mode off, turn off rx flow if already on */
	...
	case IOV_GVAL(IOV_RXFLOWHI):
		/* Used in rx flow control water mark */
...
deliver:
	...
	if (bus->rxflow_mode == RX_FLOW_CONTROL_RXBOUND) {
		...
		/* Track rx limit exceed cases in the past 4 times dpc schduled */
	...
	if ((bus->idletime > 0) && (bus->clkstate == CLK_AVAIL)) {
		if (++bus->idlecount >= bus->idletime) {
			...
			if (bus->activity) {
				bus->activity = FALSE;
#if (!(defined(UNDER_CE) && defined(NDIS60)))
				dhd_os_wd_timer(bus->dhd, dhd_watchdog_ms);  /* Greg */
			} else {
#endif /* (!(defined(UNDER_CE) && defined(NDIS60))) */
				dhdsdio_clkctl(bus, CLK_NONE, FALSE);
			}
		}
	}
...
#ifdef BCMSPI
	/* have a buffer for BCMSPI to ack each of the rxed packetx */
	DHD_INFO(("%s: completed!!\n", __FUNCTION__));
...
test_f2_loopback(dhd_bus_t *bus, void *sdh)
{
	...
	int ret = 0;
	uint32 swheader;
	uint32 dstatus; /* gSPI device status bits */

	if (dhdsdio_set_siaddr_window(bus, SI_ENUM_BASE)) {
	    DHD_ERROR(("%s: FAILED to return to SI_ENUM_BASE\n", __FUNCTION__));
	return;
	}
	/* bring socram out-of-reset */
	...
	/* Allow CM3 to settle after reset */
	OSL_DELAY(200000);
	/* Peek on F2-rx-ready bit. */
	dstatus = bcmsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL);
...
#endif /* TEST_DWORDMODE_F2_LOOPBACK */
